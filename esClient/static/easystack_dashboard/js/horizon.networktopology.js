...
horizon.network_topology = {
  ...
  init:function() {
    ...
    self.balloon_loadbalancerTmpl = Hogan.compile(angular.element('#balloon_loadbalancer').html());
  ...
  retrieve_network_info: function(force_start) {
    ...
    angular.element.getJSON(
      ...
      function(data) {
        ...
        if(!angular.equals(self.old_data, data) || force_start) {
        ...
        nodeEnter.append('path')
          ...
          .attr('transform', function(d) {
            switch (Object.getPrototypeOf(d.data)) {
              ...
              case LoadBalancer.prototype:
                return 'scale(0.027)translate(-500,-550)'
              default:
  ...
  load_topology: function(data) {
    ...
    var net, _i, _netlen, _netref, rou, _j, _roulen, _rouref, port, _l, _portlen, _portref,
        ser, _k, _serlen, _serref, fire, _m, _firelen, _fireref, lb, _n, _lblen, _lbref, obj, vmCount;
    var change = false;
    var filterNode = function(obj) {
      return function(d) {
        return obj == d.data;
    ...
    // LoadBalancer
    // lb, _n, _lblen, _lbref,
    _lbref = data.loadbalancers;
    for (_n = 0, _lblen = _lbref.length; _n < _lblen; _n++) {
      lb = _lbref[_n];
      var loadbalancer = new LoadBalancer(lb);
      if (!self.already_in_graph(self.data.loadbalancers, loadbalancer)) {
        self.new_node(loadbalancer);
        change = true;
      } else {
        obj = self.find_by_id(loadbalancer.id);
        if (obj) {
          obj.data = loadbalancer;
        }
      }
      self.data.loadbalancers[loadbalancer.id] = loadbalancer;
    }
    ...
    for (_l = 0, _portlen = _portref.length; _l < _portlen; _l++) {
      ...
      if (!self.already_in_graph(self.data.ports, port)) {
        ...
        var port_device_owner = port.device_owner.substr(0, 8);
        if (angular.isDefined(device) && angular.isDefined(_network)) {
          if (port_device_owner == 'compute:') {
          ...
          change = true;
        } else if (angular.isDefined(_network) && port_device_owner == 'compute:') {
  ...
  show_balloon: function(d,d2,element) {
    ...
    var loadbalancerTmpl = self.balloon_loadbalancerTmpl;
    ...
    var htmlData = {
      ...
      instances_label: gettext('Instances'),
      members_label: gettext('Members'),
      rules_label: gettext('Rules'),

      rule_name: gettext('Name'),
    ...
    if (d instanceof Router) {
      ...
      for(var i=0; i<d.rules.length; i++){
        var actions = d.rules[i].action;
        d.rules[i].action = gettext((actions.slice(0, 1).toUpperCase() + actions.slice(1)));
      }

      htmlData.rules = d.rules
    ...
    else if (d instanceof LoadBalancer) {
      htmlData.delete_label = gettext('Delete Loadbalancer');
      htmlData.view_details_label = gettext('View Loadbalancer Details');
      htmlData.console_id = d.id;
      if (d.instances) {
        htmlData.instances = d.instances.map(function(instance) {
          return {'id': instance};
        });
      }
      html = balloonTmpl.render(htmlData,{
        table1:deviceTmpl,
        table2:loadbalancerTmpl
      });
    }
    else {
      return;
  ...
  svgs: function(name) {
    switch (name) {
      ...
      case 'loadbalancer':
        return 'M846.472747 705.884802l0-75.718519L534.180162 630.166283l0 75.718519 67.964928 0c19.965727 0 36.316114 16.616446 36.316114 36.34272l0 1.029446 0 178.973184c0 19.965727-16.350386 36.292578-36.316114 36.292578l-1.029446 0L422.381913 958.52273c-19.965727 0-36.580127-16.32685-36.580127-36.292578l0-1.052982L385.801786 742.227522c0-19.726274 16.614399-36.34272 36.580127-36.34272l0.766456 0 66.935482 0 0-75.718519L185.570439 630.166283l0 75.718519 67.964928 0c19.727297 0 36.316114 16.616446 36.316114 36.34272l0 1.029446 0 178.973184c0 19.965727-16.588817 36.292578-36.316114 36.292578l-1.029446 0L73.509201 958.52273c-20.205181 0-36.555567-16.32685-36.555567-36.292578l0-1.052982L36.953633 742.227522c0-19.726274 16.350386-36.34272 36.555567-36.34272l1.055029 0 66.934458 0 0-97.503685 0-0.26299c0-12.18553 9.839089-22.048155 21.76163-22.048155l326.823534 0L490.083851 433.047834l-125.539204 0c-20.205181 0-36.316114-16.614399-36.316114-36.843116l0-0.766456L328.228533 101.028974c0-20.228717 16.110933-36.292578 36.316114-36.292578l1.029446 0L659.719366 64.736397c20.228717 0 36.580127 16.063861 36.580127 36.292578l0 1.052982 0 294.122761c0 20.228717-16.350386 36.843116-36.580127 36.843116l-1.028423 0L534.180162 433.047834l0 153.022139 334.077751 0 0.527003 0c11.921517 0 21.497617 9.862625 21.497617 22.048155l0 97.766674 60.473303 0c19.964704 0 36.291554 16.616446 36.291554 36.34272l0 1.029446 0 178.973184c0 19.965727-16.32685 36.292578-36.291554 36.292578l-1.055029 0L770.990612 958.52273c-20.49273 0-36.580127-16.32685-36.580127-36.292578l0-1.052982L734.410486 742.227522c0-19.726274 16.087397-36.34272 36.580127-36.34272l0.766456 0L846.472747 705.884802 846.472747 705.884802zM914.173662 779.047102 914.173662 779.047102 807.570739 779.047102l0 106.602923 106.602923 0L914.173662 779.047102 914.173662 779.047102zM216.71681 779.047102 216.71681 779.047102 110.352317 779.047102l0 106.602923 106.36347 0L216.715787 779.047102 216.71681 779.047102zM565.30095 779.047102 565.30095 779.047102 458.96204 779.047102l0 106.602923 106.339934 0L565.301973 779.047102 565.30095 779.047102zM623.426789 137.872091 623.426789 137.872091 401.38674 137.872091l0 222.016513 222.040049 0L623.426789 137.872091 623.426789 137.872091z'
      default:
        return '';
    }
  },
