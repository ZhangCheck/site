...
(function () {
  ...
  angular
    ...
    .controller('AwsLaunchInstanceFlavorController', AwsLaunchInstanceFlavorController);

  AwsLaunchInstanceFlavorController.$inject = [
    '$scope',
    'horizon.framework.widgets.charts.quotaChartDefaults',
    'AwslaunchInstanceModel'
  ];

  function AwsLaunchInstanceFlavorController($scope, quotaChartDefaults, AwslaunchInstanceModel) {
    var ctrl = this;
    ...
    ctrl.lenovoFlavorStepConfig = AwslaunchInstanceModel.lenovoFlavorStepConfig || [];
    ctrl.lenovoFlavorVolumeConfig  = AwslaunchInstanceModel.lenovoFlavorVolumeConfig || [];
    ...
    ctrl.flavors = [];
    ctrl.metadataDefs = AwslaunchInstanceModel.metadataDefs;
    ctrl.novaLimits = {};
    ...
    $scope.$watch(function () {
      return AwslaunchInstanceModel.novaLimits;
    ...
    $scope.$watchCollection(function() {
      return AwslaunchInstanceModel.flavors;
    }, function (newValue, oldValue, scope) {
      var ctrl = scope.selectFlavorCtrl;
    ...
    $scope.$watch(function () {
      return AwslaunchInstanceModel.newInstanceSpec.instance_count;
    }, function (newValue, oldValue, scope) {
    ...
    $scope.$watchCollection("selectFlavorCtrl.allocatedFlavorFacades",
      function (newValue, oldValue, scope) {
        if (newValue && newValue.length > 0) {
          AwslaunchInstanceModel.newInstanceSpec.flavor = newValue[0].flavor;
          scope.selectFlavorCtrl.validateFlavor();
        } else {
          delete AwslaunchInstanceModel.newInstanceSpec.flavor;
        }
      }
    );

    $scope.$watchCollection(function() {
      return AwslaunchInstanceModel.newInstanceSpec.source;
    }, function (newValue, oldValue, scope) {
      var ctrl = scope.selectFlavorCtrl;
    ...
    function updateFlavorFacades() {
      ...
      var instancesChartData = ctrl.getChartData(
        ...
        ctrl.instanceCount,
        AwslaunchInstanceModel.novaLimits.totalInstancesUsed,
        AwslaunchInstanceModel.novaLimits.maxTotalInstances);
      ...
      for (var i = 0; i < ctrl.availableFlavorFacades.length; i++) {
        ...
        facade.vcpusChartData = ctrl.getChartData(
          ...
          AwslaunchInstanceModel.novaLimits.totalCoresUsed,
          AwslaunchInstanceModel.novaLimits.maxTotalCores);

        facade.ramChartData = ctrl.getChartData(
          ctrl.chartTotalRamLabel,
          ctrl.instanceCount * facade.ram,
          AwslaunchInstanceModel.novaLimits.totalRAMUsed,
          AwslaunchInstanceModel.novaLimits.maxTotalRAMSize);
    ...
    function getChartData(title, added, totalUsed, maxAllowed) {
      ...
      if(allowed === Infinity){
        allowed = gettext('Not limited');
      }
      if(remainingData.value === Infinity){
        remainingData.value = 0;
        remainingData.labelvalue = gettext('Not limited');
      }
      var chartData = {
        title: title,
        maxLimit: allowed,
    ...
    function getErrors(flavor) {
      ...
      var sourceType = AwslaunchInstanceModel.newInstanceSpec.source_type;
