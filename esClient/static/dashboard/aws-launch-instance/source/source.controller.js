...
(function () {
  ...
  angular
    ...
    .controller('AwsLaunchInstanceSourceController', AwsLaunchInstanceSourceController);

  AwsLaunchInstanceSourceController.$inject = [
   '$scope',
   'bootSourceTypes',
   'bytesFilter',
  ...
  function AwsLaunchInstanceSourceController($scope,
    bootSourceTypes,
    bytesFilter,
    donutChartSettings,
    dateFilter,
    ...
    $scope.searchForId = function (id){
      glanceAPI.getImage(id).success(function(image) {
        ctrl.tableData.displayedAvailable = [image];
      });
    }
    //ctrl.sourceDetails =
    ...
    var tableHeadCellsMap = {
      image: [
        ...
        { text: gettext('Size'), style: { width: '30%' }, classList: ['number'], sortable: true },
      ...
      snapshot: [
        ...
        { text: gettext('Type'), sortable: true },
        { text: gettext('Instance Name'), sortable: true }
      ],
      volume: [
        { text: gettext('Name'), style: { width: '40%' }, sortable: true, sortDefault: true },
    ...
    function updateChart() {
      ...
      var remaining
      if(maxTotalInstances != Infinity){
        remaining =  Math.max(0, maxTotalInstances - totalInstancesUsed - added);
      }else{
        remaining = gettext("Not limited");
      }

      ctrl.instanceStats.maxLimit = maxTotalInstances != Infinity ? maxTotalInstances : gettext("Not limited");
      data[0].value = totalInstancesUsed;
      data[1].value = added;
      data[2].value = remaining;
      var quotaCalc
      if(maxTotalInstances != Infinity){
        quotaCalc = Math.round((totalInstancesUsed + added) / maxTotalInstances * 100);
      }else{
        quotaCalc = 0;
      }
