...
(function () {
  ...
  MetadataModalHelperController.$inject = [
    '$scope',
    '$window',
    'horizon.app.metadata.modal.service'
  ];

  /**
   * @ngdoc controller
   * @name horizon.app.metadata.modal.controller:MetadataModalHelperController
   * @description
   * Helper controller used by Horizon part written in Django.
   */
  function MetadataModalHelperController($scope, $window, metadataModalService) {
    ...
    function openMetadataModal(resource, id, requireReload, propertiesTarget) {
      ...
      function onOpened() {
        $scope.$root.$broadcast('afterUpdate');
        if (requireReload) {
