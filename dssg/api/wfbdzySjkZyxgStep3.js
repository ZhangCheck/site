...
/***/ temp = function(module, exports, __webpack_require__) {runed=true;
	...
	(0, _jsonp2["default"])("wfbdzySjkZyxgStep3")(function (args, callback) {
	    ...
	    var options = [];
	    sourceColumns.forEach(function (item) {
	        options.push({
	            key: item.showName,
	            value: item.showName
	        });
	    ...
	    var columns = [{
	        ...
	        "render": { "type": "select", options: options, props: { style: { width: 200 },
	                onChange: function onChange(value, text, record, index, that) {
	                    ...
	                    try {
	                        for (var _iterator = (0, _getIterator3["default"])(args.options), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	                            var item = _step.value;

	                            if (value == item.columnCode) {
	                                console.log('tableFieldName=' + value);
	                                rows.dataType = item.type;
	                                rows.dataSource = args.tables.content[args.tableIds[0]].name;
