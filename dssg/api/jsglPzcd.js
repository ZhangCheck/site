...
/***/ temp = function(module, exports, __webpack_require__) {runed=true;
	...
	var _TreeSelected = __webpack_require__(1327);
	                    ...
	                    "buttons": [{ "type": "button", "value": "取消", props: { "style": { "float": "right" } } }, {
	                        "type": "button", "value": "确定", props: {
	                            type: "primary", "style": { "float": "right" }, onClick: function onClick() {
	                                ...
	                                var value = form.getFieldValue('roleid');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _rcTree = __webpack_require__(1021);

	var _rcTree2 = _interopRequireDefault(_rcTree);

	var _openAnimation = __webpack_require__(895);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _css = __webpack_require__(962);

	var _tree = __webpack_require__(896);

	var _tree2 = _interopRequireDefault(_tree);

	var _css2 = __webpack_require__(378);

	var _treeSelect = __webpack_require__(320);
	...
	var CustomComponent = function (_Component) {
	    ...
	    function CustomComponent(props) {
	        ...
	        _this.onSelect = function (info, e) {

	            console.log('selected', info);
	        };

	        _this.onCheck = function (value, info) {
	            /*// debugger;
	            // let result = info.checkedNodesPositions.length == 1?true:false;
	            let lastPos = info.checkedNodesPositions;
	            // if(result){
	            //     lastPos = info.checkedNodesPositions[0].pos;
	            // }else {
	            //     lastPos = info.checkedNodesPositions[info.checkedNodesPositions.length-1].pos
	            // }
	            let allId = [];
	            let parentIds = info.halfCheckedKeys;
	            // let parentId;
	            ...
	            // }
	              let arrs = Global.getState().aiForm.jsglPzcd.defaultTreeData.data.myMenu;
	            let newArrs = [];
	            let flag = false;
	            ...
	            if(!flag){
	            if(value.length>1){
	            arrs.push(value[value.length-1]);
	            arrs.forEach((items)=>{
	            newArrs.push(items);
	            })
	            }else{
	            ...
	            newArrs.push(items);
	                ...
	                // newArrs.push(parentId);
	                // allId = newArrs.concat(parentIds);
	            }else{
	                arrs.forEach((item)=>{
	                    if(value == item){
	                        return;
	            ...
	            // lastPos.forEach((items)=>{
	                // let pos = items.pos;
	                // let posIds = pos.split('-')[2];
	                // let allMenu = Global.getState().aiForm.jsglPzcd.defaultTreeData.data.allMenu;
	                // allMenu.forEach((items,index)=>{
	                    // if(index == posIds){
	                        // newArrs.push(items.value);
	                        // return;
	                    // }
	                // })
	            // })
	            // let newArrs1 = [];
	            // newArrs1 = unique(newArrs);
	            let tempArrs = [];
	            // poses.forEach((items)=>{
	                // tempArrs.push(items.pos.split('-')[1]);
	            // })
	            ...
	            let lastArr = unique(tempArrs);
	            let allMenus = Global.getState().aiForm.jsglPzcd.defaultTreeData.data.allMenu;
	            newArrs.forEach((items0)=>{
	            allMenus.forEach((items1)=>{
	            if(items1.children.length>0){
	            items1.children.forEach((items2)=>{
	            if(items0 == items2.value){
	            tempArrs.push(items1.key);
	            }
	            })
	            }
	            })
	            })
	            let lastMenu = unique(tempArrs);*/
	            _global.Global.mark({
	                aiForm: {
	                    jsglPzcd: {
	                        defaultTreeData: {
	        ...
	        // ----- api demo ------
	        _global.Global.requestApi(_interfaces.apiDemo, _demoType.apiDemo, (0, _defineProperty3["default"])({}, rootState, (0, _defineProperty3["default"])({}, _this.props.id, { apiDemoApi: null })));
	        return _this;
	    }
	    ...
	    CustomComponent.prototype.render = function render() {
	        ...
	        var state = _global.Global.getState();
	        var treeData1 = [{
	            "value": "78e7bca05dd941759d96fdfaaac23ae9",
	            "key": "78e7bca05dd941759d96fdfaaac23ae9",
	            "label": "权限管理",
	            "children": [{
	                "value": "dbb548a725294751ae0cafe36e8c0afe",
	                "key": "dbb548a725294751ae0cafe36e8c0afe",
	                "label": "用户管理",
	                "children": []
	            }, {
	                "value": "cfe2642d68884957b137a1bb18395804",
	                "key": "cfe2642d68884957b137a1bb18395804",
	                "label": "角色管理",
	                "children": []
	            }, {
	                "value": "cfe2642d68884957b137a1bb18395823",
	                "key": "cfe2642d68884957b137a1bb1833567",
	                "label": "角色管理",
	                "children": []
	            }, {
	                "value": "cfe2642d68884957b137a1bb18395832",
	                "key": "cfe2642d68884957b137a1bb18395832",
	                "label": "角色管理",
	                "children": []
	            }]
	        }, {
	            "value": "778c8a92df5849a2ad70845165aa1222",
	            "key": "778c8a92df5849a2ad70845165aa1222",
	            "label": "系统管理",
	            "children": [{
	                "value": "ba29ffdcf44546c5a5d92afd242ebac8",
	                "key": "ba29ffdcf44546c5a5d92afd242ebac8",
	                "label": "参数配置",
	                "children": []
	            }]
	        }];
	        ...
	        var tProps = {
	            ...
	            value: state.aiForm.jsglPzcd.defaultTreeData.data.myMenu,
	        ...
	        var myMenus = state.aiForm.jsglPzcd.defaultTreeData.data.myMenu;
	        var allMenus = state.aiForm.jsglPzcd.defaultTreeData.data.allMenu;
	        var parentMenus = [];
	        var lastedMenu = [];
	        allMenus.forEach(function (item) {
	            parentMenus.push(item.value);
	        });
	        myMenus.forEach(function (item) {
	            if (item == "393a4481fa3c11e6a645047d7b9b34f9") {
	                lastedMenu.push(item);
	            }
	            var flag = false;
	            parentMenus.forEach(function (item1) {
	                if (item == item1) {
	                    flag = true;
	                } else {
	                    return;
	        ...
	        //lastedMenu.push("393a4481fa3c11e6a645047d7b9b34f9");  //添加首页
	        return _react2["default"].createElement(
	            _tree2["default"],
	            { className: 'myCls', showLine: true, checkable: true,
	                defaultExpandedKeys: lastedMenu
	                //defaultSelectedKeys={lastedMenu}
	                , defaultCheckedKeys: lastedMenu,
	            ...
	            treeData.map(function (data) {
	                return _react2["default"].createElement(
	                    TreeNode,
	                    { title: data.label, key: data.value },
	                    data.children.length > 0 && data.children.map(function (data1) {
	                        return _react2["default"].createElement(TreeNode, { title: data1.label, key: data1.value });
	                    })
	                );
	            })
	        );
